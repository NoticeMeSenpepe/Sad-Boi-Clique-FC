...
const HomePage = ({ setPage, setSelectedPlayer }) => {
  ...
  React.useEffect(() => {
    const targets = { pos: 2, rate: 64, gd: 28, pts: 59 };
    let frame, start;
    const animate = (ts) => {
      if (!start) start = ts;
      const p = Math.min((ts - start) / 2200, 1);
      const ease = 1 - Math.pow(1 - p, 3);
      setPulseValues({ pos: Math.round(targets.pos * ease) || 1, rate: Math.round(targets.rate * ease), gd: Math.round(targets.gd * ease), pts: Math.round(targets.pts * ease) });
  ...
  return (
    <div style={{ minHeight: '100vh', paddingBottom: 124 }}>
      {/* HERO */}
      <div style={{ position: 'relative', minHeight: 900, marginTop: 92, overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingTop: 24, paddingBottom: 24 }}>
        ...
        <div className="sbc-hero-row" style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div onClick={() => { try { localStorage.setItem('sbc_focus_news', String(h.id)); } catch (e) {} setPage('news'); }} style={{ position: 'relative', height: 220, marginBottom: 20, cursor: 'pointer' }}>
            <div key={headlineIdx} style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', animation: 'fadeInHeadline 0.5s ease' }}>
              <div className="sbc-glow-heading" style={{ fontFamily: 'Anton, sans-serif', fontSize: 'clamp(30px, 4.6vw, 60px)', lineHeight: 0.95, color: '#fff', maxWidth: 820, textTransform: 'uppercase' }}>{h.text}</div>
              <div style={{ fontFamily: 'Roboto, sans-serif', fontSize: 15, fontWeight: 300, color: 'rgba(218,218,218,0.75)', maxWidth: 520, marginTop: 16, lineHeight: 1.6 }}>{h.sub}</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
            {[
              ...
              { label: 'LEAGUE POSITION', val: '2ND', sub: '59 pts · GD +28', color: 'var(--accent)' },
          ...
          <div onClick={() => setPage('fixtures')} className="sbc-glow-panel" style={{ '--panel-color': '#2a9d8f', marginTop: 20, maxWidth: 520, background: 'rgba(8,15,30,0.85)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: '3px solid #2a9d8f', borderRadius: 4, padding: '14px 18px', cursor: 'pointer' }}>
          ...
          {(() => {
            const spotPlayers = (window.PLAYERS || []).filter((p) => p.image);
            ...
            return (
              <div className="sbc-hero-right" style={{ flex: '0 0 30%', padding: '0 32px 0 16px', display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 780 }}>
                ...
                style={{ '--panel-color': sp.accentColor, position: 'relative', borderRadius: 8, overflow: 'hidden', flex: 1, minHeight: 0,
                  border: `1px solid ${sp.accentColor}55`,
                  boxShadow: spHov ? `0 16px 50px ${sp.glowColor}` : '0 4px 24px rgba(0,0,0,0.5)',
                  transition: 'all 0.3s', cursor: 'pointer', height: "760px" }}>
                  <img src={sp.image} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top center', display: 'block', transform: spHov ? 'scale(1.04)' : 'scale(1)', transition: 'transform 0.5s' }} alt={sp.name} />
                  ...
                  {spHov && <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, ${sp.accentColor}22, transparent)`, height: "700px" }} />}
                  {/* Rating */}
      ...
      <div style={{ background: 'transparent', padding: '160px 64px 40px' }}>
        ...
        <div style={{ display: 'flex', gap: 20, overflowX: 'auto', overflowY: 'visible', padding: '12px 0 16px' }}>
          {signings.map((s, i) => {
...
Object.assign(window, { PlayerProfileModal, HomePage, SquadPage, StatsPage, FixturesPage, NewsPage, TransfersPage, LeaguePage, StorePage, AccountPage, BasketPage });